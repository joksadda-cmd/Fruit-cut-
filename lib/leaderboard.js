...
//
// Also backs the "150 slashes/week" gate for the weekly referral reward
// (see lib/referral.js) — that check reads the same weekly win count.

const { getCollection } = require('./db');
const { TRANSACTION_TYPES, LEADERBOARD_RANK_WEIGHTS } = require('./constants');
...
async function recordSlashWin(telegramId, username) {
  ...
  await col.updateOne(
    { weekKey, telegramId: String(telegramId) },
    {
      ...
      $setOnInsert: { weekKey, telegramId: String(telegramId), createdAt: new Date() },
    },
    { upsert: true }
  );
  return weekKey;
...
// Pays out the weekly prize pool to the top N, split by
// LEADERBOARD_RANK_WEIGHTS. Idempotent per week — safe to call more than
// once (e.g. a retried cron hit) because of the `distributed` flag stored
// on a small marker doc in the same collection.
async function distributeWeeklyPrizes(weekKey, prizePoolFc, topN) {
  const col = await getCollection('leaderboard');
  const markerId = `marker_${weekKey}`;

  const claimed = await col.findOneAndUpdate(
    { _id: markerId, distributed: { $ne: true } },
    { $set: { _id: markerId, weekKey, distributed: true, distributedAt: new Date() } },
  ...
  const top = await getTopSlashers(weekKey, topN);
  ...
  for (let i = 0; i < top.length; i++) {
    const entry = top[i];
    const weightPct = LEADERBOARD_RANK_WEIGHTS[i] || 0;
    if (weightPct <= 0) continue;
    const prize = Math.round((prizePoolFc * weightPct) / 100);
    if (prize <= 0) continue;
    ...
    paid.push({ telegramId: entry.telegramId, rank: i + 1, prize });
  }
