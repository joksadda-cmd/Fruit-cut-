...
// Must mirror getTopSlashers' exact sort order ({wins:-1, lastWinAt:1}) —
// otherwise "Your rank" can disagree with where the user actually appears
// in the rendered list. (Bug found & fixed: this used to count only
// strictly-higher wins, so every user tied on wins got the SAME rank
// number, while the list itself broke those same ties by lastWinAt and
// showed them at different positions — e.g. four users tied at 2 wins all
// showed "Your rank: #4" even though the list correctly placed them at
// #4, #5, #6, #7.)
async function getUserRank(telegramId, weekKey = getWeekKey()) {
  ...
  const higherCount = await col.countDocuments({
    ...
    $or: [
      { wins: { $gt: me.wins } },
      { wins: me.wins, lastWinAt: { $lt: me.lastWinAt } },
    ],
  });
...
// LEADERBOARD_MIN_WINS_FOR_PRIZE (50 wins) that week actually get paid — a
...
async function distributeWeeklyPrizes(weekKey) {
  ...
  for (let i = 0; i < top.length; i++) {
    ...
    if (prize <= 0) continue;
    if (!entry.wins || entry.wins < LEADERBOARD_MIN_WINS_FOR_PRIZE) continue; // below the 50-win eligibility gate
