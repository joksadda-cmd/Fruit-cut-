...
// Every network enforces the SAME 10s minimum server-side, regardless of
// each network's own completion guarantee — this is a deliberate product
// requirement (not a technical necessity for Adsgram, which already
// guarantees full-watch via its own promise), so it applies uniformly.
const MIN_WATCH_MS = {
  adsgram: 10000,
  adsgramDaily: 10000,
  gigapub: 10000,
  monetag: 10000,
  // Gate-only ad shown before claiming a Fruit Slash game reward — no FC
  // is paid for this ad itself, it just has to be watched before the
  // slash reward can be claimed.
  slashGate: 10000,
};
...
async function claimAdSession(telegramId, sessionId, expectedNetwork) {
  ...
  const minRequired = MIN_WATCH_MS[expectedNetwork] ?? 10000;
...
// ── Reopen a session whose Fruit Coin credit failed AFTER it was marked claimed ──
// claimAdSession() flips pending -> claimed to atomically stop double-claims.
// But the actual Fruit Coin credit (creditFcForAd) happens as a SEPARATE step
