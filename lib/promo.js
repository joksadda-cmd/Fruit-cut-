...
async function createPromoCode({ rewardFc, rewardGold, maxUses, createdBy }) {
  ...
  const rewardFcSafe = Number(rewardFc) || 0;
  const rewardGoldSafe = Number(rewardGold) || 0;

  await col.insertOne({
    code,
    rewardFc: rewardFcSafe,
    rewardGold: rewardGoldSafe,
  ...
  return { code, rewardFc: rewardFcSafe, rewardGold: rewardGoldSafe, maxUses: maxUses || 0, expiresAt };
}
...
async function redeemPromoCode(telegramId, rawCode) {
  ...
  return { ok: true, rewardFc: updated.rewardFc || 0, rewardGold: updated.rewardGold || 0, code };
}
