// lib/adReward.js
// Shared logic for crediting Fruit Coin from a confirmed ad-network postback.
// Used by api/postback.js for ALL networks (adsgram, monetag, gigapub, ...).
//
// GOLD REMOVED (2026-09): ad rewards used to credit Gold, which then had to
// be converted to Fruit Coin. Ads now credit Fruit Coin directly.
//
...
async function creditFcForAd(telegramId, amount, source, eventKey, dailyLimitMax) {
  ...
  const updatedUser = await usersCol.findOneAndUpdate(
    ...
    { $inc: { fruitCoin: amount, totalAdsWatched: 1 }, $set: { lastActive: new Date() } },
  ...
  return { success: true, newFruitCoin: updatedUser.fruitCoin };
}

module.exports = { creditFcForAd };
