...
//  - Adsgram (Special & Daily): the SDK itself requires the user to click
//    the ad creative within its ~15s window before it resolves as
//    successful — that's Adsgram's own click-required format, enforced by
//    their SDK, not something we verify ourselves. A real completion can
//    legitimately come back in just 1-2 seconds after a click, so a long
//    server-side minimum here would wrongly reject real completions. Keep
//    this just high enough to block a raw script call that skips the SDK
//    entirely and jumps straight from request_session to claim.
//  - GigaPub / Monetag: no click requirement — purely time-based. Reward
