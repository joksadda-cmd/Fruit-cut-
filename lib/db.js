...
async function connectToDatabase() {
  ...
  if (!cachedClient) {
    ...
    try {
      ...
      // IMPORTANT: do NOT cache a client that failed to connect. The old
      // code set `cachedClient = new MongoClient(...)` BEFORE calling
      // connect(), so if connect() threw (a transient network blip —
      // common under launch-day traffic spikes), `cachedClient` stayed
      // set to a broken client. Every later request on that same warm
      // serverless instance then hit `if (!cachedClient)` as false,
      // skipped reconnecting entirely, and reused the dead client forever
      // — the route (bot.js, auth.js, etc.) would fail silently until
      // Vercel eventually recycled that instance, which can take a
      // while. Leaving cachedClient as null here means the very next
      // request retries a fresh connection instead of staying broken.
      cachedClient = null;
      throw err;
    }
  }
