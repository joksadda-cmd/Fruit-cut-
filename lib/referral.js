...
// Step 1: Friend confirms channel & group join -> Referrer gets: 30 FC + 1 Game Token (clamped to 10 max)
...
// Total per complete referral: 430 FC + 1 Game Token
...
async function awardReferralMilestone(referrerId, referredUser, step, rewardFc, rewardTokens, milestoneText) {
  try {
    const usersCol = await getCollection('users');
    const referrer = await findUserByTelegramId(usersCol, referrerId);
    if (!referrer || referrer.banned) return;

    const currentTokens = referrer.gameTokens !== undefined ? referrer.gameTokens : 3;
    const newTokens = Math.min(MAX_TOKENS, currentTokens + (rewardTokens || 0));
    ...
    let msg = `🎉 <b>Referral Reward (Step ${step}/4)!</b>\n\n` +
      `${friendName} ${milestoneText}!\n\n` +
      `🍎 +${rewardFc} Fruit Coin\n`;
    if (rewardTokens > 0) msg += `🎮 +${rewardTokens} Game Token\n`;
