...
// referral to weekly-leaderboard validity IMMEDIATELY — skipping the
// REFERRAL_WEEKLY_MIN_ACCOUNT_AGE_MS wait below (see grantWeeklyReferralValidity).
//
...
// than farmed:
//   1. Unless the referred user already passed the captcha above (fast
//      path — see the header comment on that section for exactly what it
//      does and doesn't prove), the referred account must be at least
//      REFERRAL_WEEKLY_MIN_ACCOUNT_AGE_MS old — a same-day disposable
//      account never qualifies, however fast someone grinds slashes on it.
//   2. ALWAYS, regardless of the captcha: the referred account's device
//      fingerprint / signup IP must differ from the referrer's own — a
//      referrer "inviting" an account created from their own phone/network
//      never qualifies, captcha or no captcha.
...
async function grantWeeklyReferralValidity(referredUser) {
  ...
  const captchaFastTrack = referredUser.referCaptchaPassed === true;
