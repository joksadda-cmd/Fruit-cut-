...
// self-delete via TTL).
//
// Payout: top 15 by weekly referrals share a 50,000 FC pool (see
// REFERRAL_WEEKLY_REWARDS in lib/constants.js) — decreasing payout, must
// have at least REFERRAL_MIN_FOR_PRIZE (20) referrals that week to be paid.
//
...
const { TRANSACTION_TYPES, REFERRAL_WEEKLY_REWARDS, REFERRAL_MIN_FOR_PRIZE } = require('./constants');
const { sendTelegramMessage } = require('./notify');
const { recordRecentWinner } = require('./recentWinners');
...
// This user's 1-based rank for the week, or null if they haven't referred
// anyone this week. Same tie-break fix as lib/leaderboard.js's getUserRank
// — must mirror getTopReferrers' sort ({referrals:-1, lastReferralAt:1}).
async function getUserReferralRank(telegramId, weekKey = getWeekKey()) {
  ...
  const higherCount = await col.countDocuments({
    weekKey,
    $or: [
      { referrals: { $gt: me.referrals } },
      { referrals: me.referrals, lastReferralAt: { $lt: me.lastReferralAt } },
    ],
  });
...
async function distributeWeeklyReferralPrizes(weekKey) {
  ...
  for (let i = 0; i < top.length; i++) {
    ...
    if (prize <= 0) continue;
    if (!entry.referrals || entry.referrals < REFERRAL_MIN_FOR_PRIZE) continue; // below the 20-referral eligibility gate
